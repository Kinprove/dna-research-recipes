## Sources (provenance + dates)

> Rights: every source below is a third-party work cited for attribution only — its content remains © its creator/publisher and is **not reproduced** here. The recipe text is Kinprove's own distillation (see `ai-for-dna-research.rights.json`).

1. *The AI Problems That Drive Genealogists Crazy (And How to Fix Them)* — Denyse Allen (YouTube), 2026-01-27 — <https://www.youtube.com/watch?v=-L40JRNmkYI> — Rights: cited reference, not reproduced.
2. *How Can I Use AI in Creating DNA Source Citations* — Family Locket (YouTube), 2025-02-27 — <https://www.youtube.com/watch?v=Qn4M-wSCQA4> — Rights: cited reference, not reproduced.
3. *How to Use AI for DNA Source Analysis: Original, Derivative, or Authored* — Family Locket (YouTube), 2026-03-12 — <https://www.youtube.com/watch?v=sDaDQRBN2bE> — Rights: cited reference, not reproduced.
4. *How to Use AI in Creating a Leeds Chart* — Family Locket (YouTube), 2025-02-13 — <https://www.youtube.com/watch?v=0xP-ffBtZXE> — Rights: cited reference, not reproduced.
5. *Can AI Help Interpret DNA Ethnicity Estimates* — Family Locket (YouTube), 2026-03-19 — <https://www.youtube.com/watch?v=l0_3PTB2wUI> — Rights: cited reference, not reproduced.
6. *How Can AI Help Me Paint a DNA Match from GEDmatch on DNA Painter?* — Family Locket (YouTube), 2025-04-04 — <https://www.youtube.com/watch?v=vfq9xVvr76s> — Rights: cited reference, not reproduced.
7. *Washington Family Lineage Revealed from Family Burials & Opens the Door for More* — Roberta Estes (DNAeXplained), 2024-03-28 — <https://dna-explained.com/2024/03/28/washington-family-lineage-revealed-from-family-burials-opens-the-door-for-more/> — Rights: cited reference, not reproduced.
8. *RLP 225: How Multiple Relationships Affect DNA Match Analysis* — Family Locket (Research Like a Pro podcast), 2022-10-31 — <https://familylocket.com/rlp-225-how-multiple-relationships-affect-dna-match-analysis/> — Rights: cited reference, not reproduced.
9. *DNA Color Clustering: The Leeds Method for Easily Visualizing Matches* — Dana Leeds, 2018-08-23 — <https://www.danaleeds.com/dna-color-clustering-the-leeds-method-for-easily-visualizing-matches/> — Rights: cited reference, not reproduced.
10. *Ethnicity and Physical Features are NOT Accurate Predictors of Parentage or Heritage* — Roberta Estes (DNAeXplained), 2017-07-02 — <https://dna-explained.com/2017/07/02/ethnicity-and-physical-features-are-not-accurate-predictors-of-parentage-or-heritage/> — Rights: cited reference, not reproduced.
11. *Why is my percentage for a region higher (or lower) than my family tree suggests it should be?* — Ancestry Support, n.d. (accessed 2026-07-18) — <https://www.ancestry.com/cs/dna-help/ethnicity/expectations> — Rights: cited reference, not reproduced.
12. *Why are My Ethnicity Estimate Different Than Expected?* — MyHeritage Help Center, 2025-11-16 — <https://www.myheritage.com/help/en/articles/12852702-why-are-my-ethnicity-estimate-different-than-expected> — Rights: cited reference, not reproduced.
13. *Chromosome mapping and endogamy* — Lara Diamond (guest post, DNA Painter), 2022-01-30 — <https://blog.dnapainter.com/blog/chromosome-mapping-and-endogamy/> — Rights: cited reference, not reproduced.
14. *Strategies for Overcoming Endogamy* — Nicole Elder Dyer (Family Locket), 2023-01-05 — <https://familylocket.com/strategies-for-overcoming-endogamy/> — Rights: cited reference, not reproduced.

## Credits

- Kinprove — original distillation and synthesis — FSL-1.1-MIT (kinprove-original)
